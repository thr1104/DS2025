...
void generateCodes(Node* root, string code, map<char, string>& codeMap) {
    if (!root) return;
...
map<char, string> buildHuffmanTree(const vector<int>& freq) {
    ...
    generateCodes(pq.top(), "", codeMap);
    return codeMap;
}

string encode(const string& word, const map<char, string>& codeMap) {
    string res = "";
    for (int i = 0; i < word.length(); ++i) {
        char c = word[i];
        char lower = tolower(c);
        if (codeMap.find(lower) != codeMap.end()) {
            res += codeMap.find(lower)->second;
        }
    }
    return res;
}
...
string getFullText() {
    ...
    text += "Let us not wallow in the valley of despair, I say to you today, my friends. ";
    ...
    text += "This is our hope, and this is the faith that I go back to the South with. ";
    ...
    text += "Land where my fathers died, land of the Pilgrim's pride, ";
...
int main() {
    ...
    // 构建 Huffman 编码表
    map<char, string> huffMap = buildHuffmanTree(freq);

    //输出 Huffman 编码表
    cout << "=== Huffman Code Table (letters in 'I Have a Dream') ===" << endl;
    for (char c = 'a'; c <= 'z'; ++c) {
        if (freq[c - 'a'] > 0) {
            cout << c << ": " << huffMap[c] << endl;
        }
    }
    cout << endl;

    // 原有功能：编码指定单词
    string words[] = {"dream", "freedom", "justice"};
    int n = 3;

    cout << "=== Encoded Words ===" << endl;
    for (int i = 0; i < n; ++i) {
        string code = encode(words[i], huffMap);
